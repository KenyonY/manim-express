...
a1 = Arrow(ORIGIN, vec.get_array(), buff=0).set_fill(color=GREEN)
scene.play(ShowCreation(a1))
vec.apply_quaternion(q1)
a2 = Arrow(ORIGIN, vec.get_array(), buff=0).set_fill(color=GREEN)

scene.play(Transform(a1, a2))

vec = Vec3([3, 0, 0]).apply_quaternion(q1.multi(q2))
a3 =  Arrow(ORIGIN, vec.get_array(), buff=0).set_fill(color=GREEN)
